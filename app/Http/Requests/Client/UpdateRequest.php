...
class UpdateRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'dni' => 'required|alpha_num|unique:clients,dni,'.$this->route('client')->id.'|max:11|min:9',
            'ruc' => 'alpha_num|unique:clients,ruc,'.$this->route('client')->id.'|max:11|min:9',
            'address' => 'nullable|string|max:255',
            'state' => 'nullable|string|max:50',
            'country' => 'nullable|string|max:50',
            'phone' => 'required|string|regex:/[0-9]{11}/|unique:clients,phone,'.$this->route('client')->id.'|max:14|min:11',
            'email' => 'required|string|unique:clients,email,'.$this->route('client')->id.'|max:250|email:rfc,dns'
        ];
    }
